...
void main() async {
  ...
  await for (final estimate in computePi().take(100)) {//take the first 100 values of computePi
    //for loop like js
...
Stream<double> computePi({int batch = 100000}) async* {
  ...
  var count = 0;
  while (true) {//it will repeat until the 100 values
    final points = generateRandom().take(batch);//take takes the number of numbers generated by 
    //generateRandom, takes 100000 numbers
    //print("points ${points}"); (point, point, point) returned
    final inside = points.where((p) => p.isInsideUnitCircle);//calls the getter method of the point class called isInsideUnitCircle
    ...
    count += inside.length;//length of inside variable, 78315, in the 2nd iteration whe add 78315 with the new inside (78390)
...
Iterable<Point> generateRandom([int? seed]) sync* {//returns an iterable object with the form of a Point class
  //print("seed ${seed}"); seed is optional, null in this code
  final random = Random(seed);
  while (true) {
    yield Point(random.nextDouble(), random.nextDouble());//return two dimensional position with random values
  }
}

class Point {
  final double x;//x is a double variable (number with coma and decimal) and is final
  final double y;
...
//compile=converts dart code to machine code
  ...
  bool get isInsideUnitCircle => x * x + y * y <= 1;//metodo getter isInsideUnitCircle
}